...
def prepare_dataset():
    ...
    if opt.pickle and os.path.exists(opt.pickle):
        ...
        (Xtr, ytr, Xte, yte, _,_) = pickle.load(open(opt.pickle, 'rb'))
        vectorizer_time=0
    else:
...
def prepare_classifier(Cs = [1,10,100,1000], ytr=None, yte=None):
    ...
    base_learner = prepare_learner(Cs, opt.learner)
    ...
    if (opt.method in ['LR', 'LRbin']):
        if opt.method == 'LR':
            ...
            cls = OneVsRestClassifier(base_learner, n_jobs=1)
...
if __name__ == '__main__':
    ...
    assert opt.force or not check_if_already_performed(opt.logfile, opt.dataset, opt.seed, opt.n_authors, opt.docs_by_author, opt.method), 'experiment already performed'
